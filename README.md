# Hosting a Full-Stack Application

[![CircleCI](https://dl.circleci.com/status-badge/img/gh/omar-the-junior/nd0067-c4-deployment-process-project-starter/tree/master.svg?style=svg)](https://dl.circleci.com/status-badge/redirect/gh/omar-the-junior/nd0067-c4-deployment-process-project-starter/tree/master)
#  AWS console screenshots 
![elasticbeanstalk status](/screenshots/AWS-elasticbeanstalk-status.png "elasticbeanstalk status")
![s3 bucket](/screenshots/AWS-s3.png "s3 bucket")
![RDS](/screenshots/AWS-RDS.png "RDS")
---
## [click here](https://udagram-project-bucket123.s3.amazonaws.com/index.html) to head to the website frontend
---
# CircleCI build status
![CircleCI build status](/screenshots/CircleCI-build-status.png "CircleCI build status")
# CircleCI deploy status
![CircleCI deploy status](/screenshots/CircleCI-deploy-status.png "CircleCI deploy status")
# CircleCI secret configuration
![CircleCI secret config screen ](/screenshots/CircleCI-secrets-config-screen.png "CircleCI secret config screen ")

--- 

# Documentation 
- [application dependencies](documentation/application_dependencies.md)
- [pipeline description](documentation/pipeline_description.md)
- [infrastructure description](documentation/infrastructure_description.md)




